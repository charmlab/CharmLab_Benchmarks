...
def growing_spheres_search(
    ...
    :param n_search_samples: int > 0
    :param model: model object with predict_proba method
    ...
    instance_label = np.argmax(model.predict_proba(instance))
    ...
    while (not counterfactuals_found) and (count < max_iter):
        ...
        y_candidate_logits = model.predict_proba(candidate_counterfactuals.to_numpy())
...
def feature_selection(
    ...
    for idx in mutable_indices:
        ...
        temp_counterfactual[idx] = instance[0][idx]

        # Generate new counterfactual with the smallest change
        new_label = np.argmax(model.predict_proba(temp_counterfactual))

        if new_label != instance_label:
            candidate_counterfactual[idx] = instance[0][idx]
