...
from method.catalog.CLUE.library.clue_ml.src.probability import decompose_entropy_cat, decompose_std_gauss
from method.catalog.CLUE.library.clue_ml.src.utils import BaseNet, Ln_distance, MNIST_mean_std_norm, generate_ind_batch
...
class CLUE(BaseNet):
    ...
    def update_stopvec(cost_vec, it_mask, step_idx, n_early_stop, min_steps):
        ...
        if (it_mask == 0).sum() == 0 and n_early_stop > 0:
            logging.debug("iteration %d, all conditions met, stopping" % step_idx)
        return it_mask
