...
from method.catalog.CFRL.cfrl_backend import set_seed
from method.catalog.CFRL.cfrl_tabular import (
    CounterfactualRLTabular,
    apply_category_mapping,
    get_conditional_dim,
    get_he_preprocessor,
)
from method.catalog.CFRL.model import HeterogeneousDecoder, HeterogeneousEncoder

LOGGER = logging.getLogger(__name__)
LOCAL_ADULT_PATH = REPO_ROOT / "data" / "catalog" / "adult_cfrl" / "adult_cfrl.csv"
...
def fetch_adult(
    ...
    """
    Loads the locally frozen pre-processed 'adult' dataset used by the
    source CFRL reproduce pipeline.
    ...
    if not LOCAL_ADULT_PATH.exists():
        raise FileNotFoundError(
            f"Local adult_cfrl dataset not found at {LOCAL_ADULT_PATH}"
        )

    raw_data = pd.read_csv(LOCAL_ADULT_PATH).fillna("?")
    labels = raw_data["Target"].astype(int).values
    features_drop += ["Target"]
    data = raw_data.drop(features_drop, axis=1, errors="ignore")
    ...
    for feat in categorical_features:
        ...
        data_tmp = encoder.fit_transform(data[feat].values)
        data[feat] = data_tmp
...
def run_experiment() -> Dict[str, object]:
    ...
    LOGGER.info("Random forest accuracy on test split: %.3f", accuracy)
    ...
    category_map[len(feature_names) - 1] = (
        adult.target_names
    ...
    LOGGER.info("Metrics: %s", metric_summary)
    return {
...
if __name__ == "__main__":
    ...
    LOGGER.info("CFRL reproduction comparison passed.")
